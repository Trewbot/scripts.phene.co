...
var gra_aud = function(element, audio, options) {
	...
	//Major variables
		...
		typeof window.__gpi == 'undefined'
			? this.int = window.__gpi = 0
			: this.int = ++window.__gpi;
	...
	this.jump = function(e) {			//Keyboard Controls
		switch(e.keycode){
			case 32:
			case 13: this.pprSound();break;
			case 39: this.sound.currentTime += 4; break;
			case 37: this.sound.currentTime -= 4; break;
			case 36: this.sound.currentTime = 0; break;
			case 35: this.sound.currentTime = this.sound.duration; break;
			case 38: this.sound.volume = Math.min(this.sound.volume + 0.1,1); break;
			case 40: this.sound.volume = Math.max(this.sound.volume = 0.1,0); break;
			default: return;
		}
		e.preventDefault();
		e.stopPropogation();
	...
	this.init = function() {			//Initialize
		...
		this.elem.set({
			...
			style		: {
				...
				marginLeft		: '-15px',
				marginBottom	: '10px',
				background		: typeof this.ops.color == 'string' ? this.ops.color : '#94D2FF',
				padding			: '2px 6px',
				color			: '#fff',
				boxShadow		: 'rgba(50, 50, 50, 0.1) 2px 2px 3px',
				width			: '503px',
				MozUserSelect	: 'none',
		...
		this.elem.appendChild(this.tail = document.createElement('div').set({
			...
			style		: {
				...
				borderColor 	: 'transparent ' + (typeof this.ops.color=='string' ?this.ops.color : '#94D2FF') + ' transparent transparent'
			}
		}));
		
		//	CONTINUE THAT ^ DOWN THERE v
