 ...
 *		v0.4.2.0017				Jan 06, 2014		Added copy link locations
 *		v0.4.2.0018				Jan 06, 2014		Fixed copy link location for blank links
 *		v0.4.2.0019				Jan 06, 2014		Added link location copying back in for blank links, switching the adress with the current loction
 *		v0.4.2.0019.5			Jan 15, 2014		Added quick fix for Graphene test build w0.4.0.0419
 */
...
if(typeof Graphene !== "object") var Graphene = new(function () {
	this.cm = true;
	this.v = "v0.4.2.0019";
...
Graphene.context = {
	...
	open : function (e, o) {
		...
		for (var op in dops)
			c.innerHTML += dops[op] !== '' ? '<div class="context-option" onclick="' + dops[op] + ';Graphene.context.close();">' + op + '</div>' : '<div class="context-disabled">' + op + '</div>';
		c.innerHTML += '<a href="view-source:' + window.location + '" target="_blank" onclick="Graphene.context.close();"><div class="context-option">View Source</div></a>';
		...
		if (pa = el.parentAnchor()) {
			...
			if (~hr.indexOf(Graphene.url))
				c.innerHTML += '<a href="' + hr + '" permalink onclick="Graphene.context.close();"><div class="context-option">Open Link as Permalink</div></a>';
			c.innerHTML += '<a href="' + hr + '" target="_blank" onclick="Graphene.context.close();"><div class="context-option">Open Link in New Tab</div></a>';
		...
		if(text !== '') {
			...
			c.innerHTML += '<a href="https://www.google.com/search?q=' + encodeURIComponent(text) + '" target="_blank" onclick="Graphene.context.close();"><div class="context-option">Search Google for "' + (text.length > 16 ? text.substring(0,15) + '...' : text) + '"</div></a>';
			c.innerHTML += '<div style="margin:2px 4px;height:1px;background:#ddd;"></div>'
		}
		
		//	Script Set Options
		for (var op in o)
			c.innerHTML += o[op] !== '' ? '<div class="context-option" onclick="' + o[op] + ';Graphene.context.close();">' + op + '</div>' : '<div class="context-disabled">' + op + '</div>';
		if (o.length > 0)
			c.innerHTML += '<div style="margin:2px 4px;height:1px;background:#ddd;"></div>';

		//	Dev Options
		c.innerHTML += '<a href="' + this.url + '/changes" onclick="Graphene.context.close();"><div class="context-disabled"><i>' + (this.cm ? 'Context Menu' : 'Graphene') + ' ' + this.v + '</i></div></a>';
		...
		if(hr !== false)
			swfobject.embedSWF("http://api.phene.co/gra_cm_flash/copy.swf", "context-link-copy", "200", "21", "0.0.1", false, {cBoard:hr!=''?hr:location.href}, {wMode:'transparent'}, {}, function(e){
				e.ref.style.opacity = 0;
				e.ref.style.position = 'absolute';
				e.ref.style.marginTop = '-2px';
				e.ref.style.left = '2px';
...
document.documentElement.oncontextmenu = Graphene.context.open;
document.addEventListener('scroll', Graphene.context.close);
document.addEventListener('contextmenu', Graphene.context.close);
